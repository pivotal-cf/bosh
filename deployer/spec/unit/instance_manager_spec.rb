...
describe Bosh::Deployer::InstanceManager do
  ...
  it "should have an HTTP client attached to the deployer" do
    @deployer.http_client.should be_instance_of(HTTPClient)
  end
  ...
  context "with a successful cloud/agent" do
    before do
      @deployer = make_deployer
      @cloud = fake_successful_cloud
      Bosh::Deployer::Config.stub!(:cloud).and_return(@cloud)
      @agent = fake_successful_agent
      Bosh::Deployer::Config.stub!(:agent).and_return(@agent)
    end
    ...
    it "should create a Bosh instance" do
      ...
      @agent.should_receive(:run_task).with(:apply, spec)
      @agent.should_receive(:run_task).with(:start)
      @deployer.http_client.should_receive(:get).and_return(stub(body: "{}"))
    ...
    it "should retry after 0.5 seconds or so if there is a HTTPClient::ConnectTimeoutError" do
      spec = YAML.load_file(spec_asset("apply_spec.yml"))
      @deployer.stub!(:run_command)
      @deployer.stub!(:load_apply_spec).and_return(spec)
      @deployer.stub!(:load_stemcell_manifest).and_return({})

      @deployer.http_client.should_receive(:get).ordered.and_raise(HTTPClient::ConnectTimeoutError)
      @deployer.http_client.should_receive(:get).ordered.and_return(stub(body: "{}"))
    ...
    def fake_successful_cloud
      cloud = mock("cloud")
      cloud.stub(:create_stemcell)
      cloud.stub(:create_vm)
      cloud.stub(:create_disk)
      cloud.stub(:attach_disk)
      cloud
    end

    def fake_successful_agent
      agent = mock("agent")
      agent.stub(:run_task)
      agent.stub(:ping)
      agent
